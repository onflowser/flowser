    ...
    async useProject(id: string) {
        ...
        if (this.currentProject.emulator) {
            ...
            try {
                ...
                throw new ServiceUnavailableException(`Can not start emulator with project ${id}`, e.message);
            }
        ...
        return Promise.all(
            projects.map(async (project) => {
                if (project.gateway) {
                    ...
                    const pingable =
                        project.isOfficialNetwork() || (await FlowGatewayService.isPingable(address, port));
