...
export class FlowEmulatorService {
    ...
    isStarted() {
        return (
            !this.emulatorProcess?.killed && [FlowEmulatorState.STARTED, FlowEmulatorState.RUNNING].includes(this.state)
        );
    }

    isRunning() {
        return !this.emulatorProcess?.killed && this.state === FlowEmulatorState.RUNNING;
    }
