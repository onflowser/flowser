...
export class FlowCliOutput {
    ...
    parse() {
        return FlowCliOutput.sanitiseOutput(this.out).split('\n').map(FlowCliOutput.parseLine).filter(Boolean);
    }
...
export class FlowCliService {
    ...
    async execute(bin = '', args): Promise<FlowCliOutput> {
        ...
        return new Promise((resolve, reject) => {
            ...
            process.on('exit', (code) => (code === 0 ? resolve(new FlowCliOutput(out)) : reject(out)));
        });
    }
