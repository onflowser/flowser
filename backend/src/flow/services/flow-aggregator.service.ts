...
export class FlowAggregatorService {
    ...
    async fetchDataFromDataSource(): Promise<void> {
        ...
        const startBlockHeight = lastStoredBlock ? lastStoredBlock.height + 1 : initialStartBlockHeight;
        ...
        let data;
        try {
            data = await this.flowGatewayService.getBlockDataWithinHeightRange(startBlockHeight, endBlockHeight);
        } catch (e) {
    ...
    async handleTransactionCreated(tx: Transaction) {
        return Promise.all([this.transactionService.create(tx), this.updateAccount(`0x${tx.payer}`)]);
    }
