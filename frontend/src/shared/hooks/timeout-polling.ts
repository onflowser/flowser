    ...
    const { isFetching, error } = useQuery<{ data: T[] }, Error>(resource, fetchCallback, {
        onSuccess: (response: any) => {
            if (response.status === 200 && response.data?.data?.length) {
                ...
                if (resourceId) {
                    ...
                    if (updatedItems.length) {
                        updatedItems.forEach((updatedItem: any, index: number) => {
                            if (newData.length === 0) {
                                newData.push({ ...updatedItem, isUpdated: false, isNew: false });
                            } else {
                                ...
                                if (i !== -1) {
                                    newData[i] = { ...updatedItems[index], isUpdated: !!data.length, isNew: false };
                                } else {
                                    newData.push({ ...updatedItems[index], isUpdated: false, isNew: !!data.length });
                                }
                            }
                        });
                        setData(newData);
                ...
                if (newestFirst) {
                    const sortedData = [...newData, ...remapDataIsNew(newItems, !!data.length)].sort((a, b) => {
                        return b.createdAt - a.createdAt;
                    });
                    setData(sortedData);
                } else {
                    setData((state: any) => [...newData, ...remapDataIsNew(newItems, !!state.length)]);
                }
            }
        },
        enabled: !stop,
